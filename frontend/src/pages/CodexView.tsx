...
import md, { extractHeadings, HeadingInfo, NestedHeading, createNestedHeadings } from '../utils/markdown';
...
export const CodexView: React.FC = () => {
  ...
  const [sectionHeadings, setSectionHeadings] = useState<HeadingInfo[]>([]);
  const [expandedSections, setExpandedSections] = useState<Set<number>>(new Set());
  const [expandedH2Headings, setExpandedH2Headings] = useState<Set<string>>(new Set());
  const [allSectionHeadings, setAllSectionHeadings] = useState<Map<number, HeadingInfo[]>>(new Map());
  const [allNestedHeadings, setAllNestedHeadings] = useState<Map<number, NestedHeading[]>>(new Map());
  ...
  const loadCodex = async (codexId: number) => {
    try {
      ...
      if (data.sections && data.sections.length > 0) {
        ...
        const selectedSec = firstIncomplete || data.sections[data.sections.length - 1];
  ...
  const getSectionHeadings = async (sectionId: number): Promise<HeadingInfo[]> => {
    try {
      const content = await sectionAPI.getContent(sectionId);
            ...
            {codex.sections && codex.sections.length > 0 ? (
              <div className="p-2 flex flex-col items-stretch">
                {codex.sections.map((section) => {
                  const nestedHeadings = allNestedHeadings.get(section.id) || [];
                  const isExpanded = expandedSections.has(section.id);
                  const hasHeadings = nestedHeadings.length > 0;
                  
                  return (
                    <div key={section.id} className="mb-1 flex flex-col items-stretch">
                      <div
                          ...
                          {nestedHeadings.map((h2Heading) => (
                            <div key={h2Heading.id} className="mb-1">
                              ...
                              <div
                                ...
                                onClick={() => handleH2Click(h2Heading.id, h2Heading.children.length > 0)}
                              >
                                ...
                                <span className="font-medium">{h2Heading.text}</span>
                              </div>

                              {/* H3 Children */}
                              {h2Heading.children.length > 0 && expandedH2Headings.has(h2Heading.id) && (
                                <div className="ml-6 mt-1 flex flex-col space-y-0.5">
                                  {h2Heading.children.map((h3Heading) => (
                                    <div
                                      ...
                                      onClick={() => scrollToHeading(h3Heading.id)}
                                    >
